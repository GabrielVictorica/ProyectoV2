...
export function CloseTransactionDialog({ propertyId, transaction, onSuccess, trigger }: Props) {
    ...
    const [pendingStatusUpdates, setPendingStatusUpdates] = useState<{ id: string, name: string, date: string }[]>([]);
    ...
    const handleConfirmUpdate = async () => {
        ...
        const res = await updatePersonStatusAction(currentUpdate.id, 'cierre', currentUpdate.date);
        if (res.success) {
            toast.success(`CRM: ${currentUpdate.name} ahora está en "Cierre"`);
        } else {
                            ...
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-xl">
                                    {isConfirmingNo
                                        ...
                                        : `¿Actualizar estado de ${currentUpdate.name} a "Cierre"?`}
                                </AlertDialogTitle>
                                <AlertDialogDescription className="text-slate-400 text-base">
                                    {isConfirmingNo
                                        ...
                                        : `Se recomienda actualizar el estado del cliente a "Cierre" para mantener el historial del CRM al día.`}
                                </AlertDialogDescription>
                            </AlertDialogHeader>
